...
public class ColorMap
{
	...
	/**
	 * The Parula colormap. Monotonic changes in brightness, from blue to
	 * yellow.
	 */
	...
	/**
	 * The Glasbey colormap with 256 colors. The Glasbey colormap is a color
	 * table structured in a maximally discontinuous manner. That is, adjacent
	 * color bins are chosen to be as distinct from one another as possible.
	 */
	public static ColorMap GLASBEY;
	...
	static
	{
		...
		GLASBEY = ColorMap.glasbey();
		colorMaps.put( GLASBEY.name, GLASBEY );
	...
	public Color get( final double val )
	{
		...
		final int b1 = colors[ i - 1 ].getBlue();
		final int a1 = colors[ i - 1 ].getAlpha();
		final int r2 = colors[ i ].getRed();
		final int g2 = colors[ i ].getGreen();
		final int b2 = colors[ i ].getBlue();
		final int a2 = colors[ i ].getAlpha();
	...
	/**
	 * Returns the number of colors used to define this colormap.
	 *
	...
	/**
	 * Returns the non-interpolated color with the specified index in the color
	 * table used to define this color map. Returns the missing color of this
	 * colormap if index is negative or greater or equal to the number of
	 * colors.
	 *
	 * @param index
	 *            the index of the color to query.
	 * @return the color.
	 * @see #getNColors()
	 */
	public Color getColor( final int index )
	{
		return ( index < 0 || index >= colors.length ) ? missingColor : colors[ index ];
	}
	...
	public final static ColorMap jet()
	{
		final Color[] colors = new Color[] {
				new Color( 0f, 0f, 1.f ),
				new Color( 0f, 0.3333f, 1.f ),
				new Color( 0f, 0.6667f, 1.f ),
				new Color( 0f, 1.f, 1.f ),
				new Color( 0.3333f, 1.f, 0.6667f ),
				new Color( 0.6667f, 1.f, 0.3333f ),
				new Color( 1.f, 1.f, 0f ),
				new Color( 1.f, 0.6667f, 0f ),
				new Color( 1.f, 0.3333f, 0f ),
				new Color( 1.f, 0f, 0f ),
		};
	...
	private static final ColorMap parula()
	{
		...
		{
			colors[ i ] = new Color( values[ j++ ], values[ j++ ], values[ j++ ] );
	...
	private static final ColorMap viridis()
	{
		...
		final float[][] vals = new float[][] { { 0.267004f, 0.004874f, 0.329415f }, { 0.268510f, 0.009605f, 0.335427f }, { 0.269944f, 0.014625f, 0.341379f }, { 0.271305f, 0.019942f, 0.347269f }, { 0.272594f, 0.025563f, 0.353093f }, { 0.273809f, 0.031497f, 0.358853f }, { 0.274952f, 0.037752f, 0.364543f }, { 0.276022f, 0.044167f, 0.370164f }, { 0.277018f, 0.050344f, 0.375715f }, { 0.277941f, 0.056324f, 0.381191f }, { 0.278791f, 0.062145f, 0.386592f }, { 0.279566f, 0.067836f, 0.391917f }, { 0.280267f, 0.073417f, 0.397163f }, { 0.280894f, 0.078907f, 0.402329f }, { 0.281446f, 0.084320f, 0.407414f }, { 0.281924f, 0.089666f, 0.412415f }, { 0.282327f, 0.094955f, 0.417331f }, { 0.282656f, 0.100196f, 0.422160f }, { 0.282910f, 0.105393f, 0.426902f }, { 0.283091f, 0.110553f, 0.431554f },
			...
			for ( int i = 0; i < ncolors; i++ )
			{
				colors[ i ] = new Color( vals[ i ][ 0 ], vals[ i ][ 1 ], vals[ i ][ 2 ] );
				alphas[ i ] = ( double ) i / ( ncolors - 1 );
			}
	...
	private static ColorMap glasbey()
	{
		// From ImageJ
		...
		return new ColorMap( "glasbey", colors, alphas, Color.BLACK, Color.GRAY );
	}
	...
	private static void loadColorMaps()
	{
		try
		{
			final FileReader input = new FileReader( COLORMAP_FILE );
			final Yaml yaml = ColorMapIO.createYaml();
			final Iterable< Object > objs = yaml.loadAll( input );
			for ( final Object obj : objs )
			{
				final ColorMap ts = ( ColorMap ) obj;
				colorMaps.put( ts.name, ts );
			}
	...
	public static void saveColorMaps()
	{
		try
		{
			final List< ColorMap > toSave = new ArrayList<>();
			toSave.add( jet() );
			toSave.add( cubeHelix() );

			mkdirs( COLORMAP_FILE );
			final FileWriter output = new FileWriter( COLORMAP_FILE );
			final Yaml yaml = ColorMapIO.createYaml();
			yaml.dumpAll( toSave.iterator(), output );
			output.close();
		}
